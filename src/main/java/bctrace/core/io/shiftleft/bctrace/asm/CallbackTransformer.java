...
import bctrace.core.io.shiftleft.bctrace.util.Utils;
...
public class CallbackTransformer implements ClassFileTransformer {
  ...
  public byte[] transform(final ClassLoader loader,
    ...
    try {
      Set<DirectListener> directListeners = new HashSet<>();
      for (Hook hook : this.hooks) {
        if (hook.getListener() instanceof DirectListener) {
          directListeners.add((DirectListener) hook.getListener());
        }
      }
      ...
      cr.accept(cn, 0);
      HashSet<String> methodsAdded = new HashSet<>();
