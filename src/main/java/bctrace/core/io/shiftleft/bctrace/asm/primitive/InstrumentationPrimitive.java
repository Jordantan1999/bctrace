 ...
 */
package bctrace.core.io.shiftleft.bctrace.asm.primitive;
  ...
  protected ArrayList<Integer> getListenersOfType(ArrayList<Integer> hooksToUse,
    ...
    for (int h = 0; h < hooksToUse.size(); h++) {
      ...
      if (hooks[i].getListener() != null &&
        ...
        if (ret == null) {
          ret = new ArrayList<>(hooksToUse.size());
        }
        ret.add(i);
      }
    }
  ...
  protected void pushMethodArgs(InsnList il, MethodNode mn) {
    ...
    if (methodArguments.length > 0) {
      ...
      for (Type methodArgument : methodArguments) {
        il.add(ASMUtils.getLoadInst(methodArgument, index));
        index += methodArgument.getSize();
      }
    }
  }
